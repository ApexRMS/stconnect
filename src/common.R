rasterFileName <- function(prefix, iteration, timestep, extension) {
    return(sprintf("%s.it%s.ts%s.%s", prefix, iteration, timestep, extension))
}

rasterFileNameSpecies <- function(prefix, species, iteration, timestep, extension) {
    return(sprintf("%s.%s.it%s.ts%s.%s", prefix, species, iteration, timestep, extension))
}

datasheetExpectData <- function(ssimObj, name) {
    ds = datasheet(ssimObj, name)
    if (nrow(ds) == 0) { stop("No data for: ", name) }
    return(ds)
}

singleValueExpectData <- function(df, name) {
    v = df[, name]
    if (is.na(v)) { stop("Missing data for: ", name) }
    return(v)
}
...
# GLOBAL_Species = GetDataSheetExpectData("stconnect_Species", GLOBAL_Project)
