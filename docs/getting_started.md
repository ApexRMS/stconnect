...
<br>
As shown in the image above, the Library you have just opened contains four Scenarios, each with a unique ID. These scenarios contain a suite of model inputs derived from historical land change data. The first two of these scenarios (with IDs=350 and 358, as shown above in square brackets) are cropped landscapes of the next two Scenarios. It is recommended that you follow the Quickstart Tutorial using the first Scenario pairing, as it will take a significantly shorter amount of time to complete a run. Explore the **Full Landscape** Scenarios afterwards if desired. The scenarios named ***4X Less Urbanization Scenario*** contain model inputs corresponding to an alternative land management plan where urbanization targets are 4 times less than those of the associated ***Baseline Scenario***.
<br>
