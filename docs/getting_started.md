...
> **Note:**
> Because you chose the ***Simple Conn. Model*** Template when you created your Library, your Library already contains four pre-configured Scenarios with model inputs. These inputs were filled in and distributed as a sample with the package to help you get started quickly, and represent hypothetical management Scenarios. The first two of these scenarios (with IDs=350 and 358, as shown below in square brackets) are cropped landscapes of the next two Scenarios. It is recommended that you follow the quickstart tutorial using the first Scenario pairing, as it will take a significantly shorter amount of time to complete a run. Explore the **Full Landscape** Scenarios afterwards if desired.
